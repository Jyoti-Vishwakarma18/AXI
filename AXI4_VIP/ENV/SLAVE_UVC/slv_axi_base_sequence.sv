...
class slv_axi_sequence#(int ADDR_WIDTH = 16 , DATA_WIDTH =32 , ID_WIDTH =16) extends uvm_sequence#(slv_axi_seq_item#( ADDR_WIDTH, DATA_WIDTH, ID_WIDTH));
   ...
   `uvm_object_param_utils(slv_axi_sequence#( ADDR_WIDTH, DATA_WIDTH, ID_WIDTH))
   `uvm_register_cb(slv_axi_sequence, slv_axi_seq_callback)
 ...
 //foreach(trans_h.wdata_array[i]) $display($time," : BEFORE STROBE WRITE_QUEUE DATA    : %0h",trans_h.wdata_array[i]);
  ...
  foreach(trans_h.write_strobe[i,j]) 
   ...
   `uvm_do_callbacks(slv_axi_sequence#( ADDR_WIDTH, DATA_WIDTH, ID_WIDTH), slv_axi_seq_callback#( ADDR_WIDTH, DATA_WIDTH, ID_WIDTH), error_wr_resp(trans_h))
 ...
 //____________________________________________READ FORM MEMORY_________________________________________________________________________________________________________
  task read_operation();
     ...
     `uvm_do_callbacks(slv_axi_sequence#( ADDR_WIDTH, DATA_WIDTH, ID_WIDTH), slv_axi_seq_callback#( ADDR_WIDTH, DATA_WIDTH, ID_WIDTH), error_rd_resp(trans_h))
